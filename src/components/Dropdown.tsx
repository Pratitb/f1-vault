...
const Dropdown = () => {
    ...
    return (
        <div className="bg-white border-2 border-red rounded-lg py-2 font-semibold text-lg cursor-pointer">
            {/* Selected Value */}
            <div className={`flex justify-between items-center gap-2 px-3 ${optionsVisible ? 'pb-2 border-b-2 border-red' : ''}`} onClick={handleOptionsVisibility}>
                <p className="text-red">{selectedYear}</p>
                <IoIosArrowDown fill="#ff1e02" />
            </div>
            {/* Options */}
            <div className={`${optionsVisible ? 'h-40 pt-2' : 'h-0'} overflow-y-auto`}>
                {seasonYears?.reverse()?.map((year: number) => <p key={year} className="py-2 rounded-lg px-4 hover:bg-red hover:text-white" onClick={() => handleOptionSelect?.(year)}>{year}</p>)}
            </div>
        </div>
    )
}
